...
async function fetchPizze() {
    ...
    const pizze_cursor = await db.collection('wa-test').find();
    pizze.value = await pizze_cursor.toArray();
}
...
<template>
    ...
    <!-- </div> -->
      <OrderFooter v-if="odabrana_pizza" :odabrana-pizza="odabrana_pizza" /> 
</template>
